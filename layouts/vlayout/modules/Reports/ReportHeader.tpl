...
{strip}
	<div class="reportsDetailHeader row-fluid">
		...
		<div class="reportHeader row-fluid span12">
			<div class='span4' style="position:relative;left:10px">
				{if $REPORT_MODEL->isEditable() eq true}
					...
					&nbsp;
				{/if}
				<button style="visibility:hidden;" onclick='window.location.href="{$REPORT_MODEL->getDuplicateRecordUrl()}"' type="button" class="cursorPointer btn"><strong>{vtranslate('LBL_DUPLICATE',$MODULE)}</strong></button>
			</div>
			...
			<div class='span4'>
				<span class="pull-right" style='margin-right:50px;'>
                	{assign var=COUNTER_NUM value=0}
					{foreach item=DETAILVIEW_LINK from=$DETAILVIEW_LINKS}
                        ...
                        {/if}
                       {assign var=COUNTER_NUM value=$COUNTER_NUM+1}
					{/foreach}
				</span>
			</div>
		</div>
		<div class="well contentsBackground span11">
            ...
            {include file='CustomFilter.tpl'|@vtemplate_path:$MODULE}
			<div class="row">
				<div class="span4 offset5">
					<input type="button" class="btn btn-success generateReport" data-mode="generate" value="{vtranslate('LBL_GENERATE_NOW',$MODULE)}"/>&nbsp;
