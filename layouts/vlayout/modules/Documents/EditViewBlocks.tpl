	...
	<form class="form-horizontal recordEditView" id="EditView" name="EditView" method="post" action="index.php" enctype="multipart/form-data">
		...
		{foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$RECORD_STRUCTURE name="EditViewBlockLevelLoop"}
			...
			{foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}
				...
				{/if}
               {if ($FIELD_MODEL->get('name') eq 'folderid' || $FIELD_MODEL->get('name') eq 'assigned_user_id' || $FIELD_MODEL->get('name') eq 'filelocationtype' || $FIELD_MODEL->get('name') eq 'filestatus' || $FIELD_MODEL->get('name') eq 'fileversion') && $SOURCE_RECORD neq '' && $SOURCE_MODULE eq 'Campaigns'}
                {else}
				<td class="fieldLabel"> 
               ...
               {if ($FIELD_MODEL->get('name') eq 'folderid' || $FIELD_MODEL->get('name') eq 'assigned_user_id' || $FIELD_MODEL->get('name') eq 'filelocationtype' || $FIELD_MODEL->get('name') eq 'filestatus' || $FIELD_MODEL->get('name') eq 'fileversion') && $SOURCE_RECORD neq '' && $SOURCE_MODULE eq 'Campaigns'}
                ...
                <td style="display:none" class="fieldValue" {if $FIELD_MODEL->get('uitype') eq '19'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if} {if $FIELD_MODEL->get('uitype') eq '20'} colspan="3"{/if}>
