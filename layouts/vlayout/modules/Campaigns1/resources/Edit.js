...
function getDeletedCurrentid(id) {
	...
	var newRow = rowno[1];	
	$("#row_1_target_check"+newRow).hide();
	$("#row_2_outlet"+newRow).hide();
	$("#row_3_leaflet"+newRow).hide();
	$("#row_4_poster"+newRow).hide();
	$("#row_5_tag"+newRow).hide();
	$("#row_6_total_uploaded_lead"+newRow).hide();
	$("#row_7_total_rejected_lead"+newRow).hide();	
}
...
Vtiger_Edit_Js("Campaigns_Edit_Js",{
	...
	isIndividualTaxMode : function() {
		...
		if(selectedOption.val() == Campaigns_Edit_Js.individualTaxType){
			return true;
		}
		return false;
	},
	...
	isGroupTaxMode : function() {
		var taxTypeElement = this.getTaxTypeSelectElement();
		var selectedOption = taxTypeElement.find('option:selected');
		if(selectedOption.val() == Campaigns_Edit_Js.groupTaxType){
			return true;
		}
		return false;
	},
	...
	calculateDiscountForLineItem : function(lineItemRow) {
		...
		if(discountType == Campaigns_Edit_Js.percentageDiscountType){
		...
		}else if(discountType == Campaigns_Edit_Js.directAmountDiscountType){
	...
	updateLineItemsElementWithSequenceNumber : function(lineItemRow,expectedSequenceNumber , currentSequenceNumber){
		...
		var idFields = new Array('productName','subproduct_ids','hdnProductId',
							   'comment','startdate','enddate','location','target_check',
							   'outlet','leaflet','poster','tag','total_uploaded_lead',
								'total_rejected_lead','day','distance','potential','budgetcost','row_1_target_check','row_2_outlet','row_3_leaflet','row_4_poster','row_5_tag','row_6_total_uploaded_lead','row_7_total_rejected_lead', 'deleteditemvalue_');
...
/*  Code modified by jitendra singh [TECHFOUR] for disable dragging
