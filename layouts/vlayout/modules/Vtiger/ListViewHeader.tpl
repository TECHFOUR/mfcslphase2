	...
	<div class="listViewPageDiv">
		<div class="listViewTopMenuDiv noprint">
			<div class="listViewActionsDiv row-fluid">
                ...
                {*<!--  Start code to hide the action button from Lead detail view for CRO and Agent by jitendra singh on 20 Feb 2014-->*}
					
                    {if $PROFILE_ID eq 3 || $PROFILE_ID eq 5 || $PROFILE_ID eq 6}
                    ...
                    <span class="btn-group listViewMassActions">
						{if count($LISTVIEW_MASSACTIONS) gt 0}
							...
							<ul class="dropdown-menu">
                                ...
                                {if $IS_ADMIN == 'off'} {*<!-- Start code to hide extra link from Action button in List View by jitendra singh on 5 feb14 -->*}
								
                                {if $LISTVIEW_MASSACTION->getLabel() eq 'LBL_EDIT'}
                                ...
                                {if $LISTVIEW_LINKS['LISTVIEW']|@count gt 0 && $IS_ADMIN != 'off'}
									<li class="divider"></li>
									{foreach item=LISTVIEW_ADVANCEDACTIONS from=$LISTVIEW_LINKS['LISTVIEW']}
										<li id="{$MODULE}_listView_advancedAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($LISTVIEW_ADVANCEDACTIONS->getLabel())}"><a {if stripos($LISTVIEW_ADVANCEDACTIONS->getUrl(), 'javascript:')===0} href="javascript:void(0);" onclick='{$LISTVIEW_ADVANCEDACTIONS->getUrl()|substr:strlen("javascript:")};'{else} href='{$LISTVIEW_ADVANCEDACTIONS->getUrl()}' {/if}>{if $LISTVIEW_ADVANCEDACTIONS->getLabel() neq 'LBL_IMPORT'}{vtranslate($LISTVIEW_ADVANCEDACTIONS->getLabel(), $MODULE)}{/if}</a></li>
									{/foreach}
								{/if}
							</ul>
						{/if}
                    ...
                    {/if}
                     {*<!--  End code to hide the action button from Lead detail view for CRO and Agent by jitendra singh on 20 Feb 2014-->*}
                    
					{foreach item=LISTVIEW_BASICACTION from=$LISTVIEW_LINKS['LISTVIEWBASIC']}
                    
                    {if $MODULE eq 'Calendar' && $LISTVIEW_BASICACTION->getLabel() neq 'LBL_ADD_EVENT'}
                     ...
                     <button id="{$MODULE}_listView_basicAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($LISTVIEW_BASICACTION->getLabel())}" class="btn addButton" {if stripos($LISTVIEW_BASICACTION->getUrl(), 'javascript:')===0} onclick='{$LISTVIEW_BASICACTION->getUrl()|substr:strlen("javascript:")};'{else} onclick='window.location.href="{$LISTVIEW_BASICACTION->getUrl()}"'{/if}><i class="icon-plus icon-white"></i>&nbsp;<strong>{vtranslate($LISTVIEW_BASICACTION->getLabel(), $MODULE)}</strong></button>
                        ...
                        <span class="btn-group">
                       {* <!--Code is Added for Quick Create by Default on 8 jan 2014-->*} 
                        {if $MODULE eq 'Leads'}
                        	<button onclick="getQuickCreateModule('menubar_quickCreate_Leads')" class="btn addButton"> <i class="icon-plus icon-white"></i>&nbsp;<strong>{vtranslate($LISTVIEW_BASICACTION->getLabel(), $MODULE)}</strong></button>
                         {else}
							<button id="{$MODULE}_listView_basicAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($LISTVIEW_BASICACTION->getLabel())}" class="btn addButton" {if stripos($LISTVIEW_BASICACTION->getUrl(), 'javascript:')===0} onclick='{$LISTVIEW_BASICACTION->getUrl()|substr:strlen("javascript:")};'{else} onclick='window.location.href="{$LISTVIEW_BASICACTION->getUrl()}"'{/if}><i class="icon-plus icon-white"></i>&nbsp;<strong>{vtranslate($LISTVIEW_BASICACTION->getLabel(), $MODULE)}</strong></button>{/if}
                            {*<!--End -->*}
...
{literal}
<script>
function getQuickCreateModule(quickleadid) {
	$('#'+quickleadid+'').click();	
}
</script>
{/literal}
