...
<div class="dashboardWidgetHeader">
	...
	<thead>
		<tr>
			<th class="span4">
				<div class="dashboardTitle"><b>&nbsp;&nbsp;{if $PROFILEID_PERMISSION eq 4 }
                											{vtranslate($WIDGET->getTitle())}
                										{else}
                                                            {if vtranslate($WIDGET->getTitle()) eq 'Todays New Calling'}
                                                            	Total Calling
                                                            {else}
                                                            	{vtranslate($WIDGET->getTitle())}
                                                            {/if}
			...
			<th class="span2">
            {if $PROFILEID eq 1 || $PROFILEID_PERMISSION ne 4 }
				<div>
