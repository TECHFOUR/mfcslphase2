...
{strip}
    ...
    {if $MODULE eq 'Users' && ($BLOCK_LABEL_KEY eq 'LBL_CURRENCY_CONFIGURATION' || $BLOCK_LABEL_KEY eq 'LBL_MORE_INFORMATION' || $BLOCK_LABEL_KEY eq 'LBL_ADDRESS_INFORMATION' || $BLOCK_LABEL_KEY eq 'LBL_USER_IMAGE_INFORMATION' || $BLOCK_LABEL_KEY eq 'LBL_USER_ADV_OPTIONS' || $BLOCK_LABEL_KEY eq 'LBL_CALENDAR_SETTINGS') && $CURRENT_USER neq '1'}
   
   {elseif $MODULE eq 'Calendar' && $BLOCK_LABEL_KEY eq 'LBL_RELATED_TO'}
				...
				{else}
				 ...
				 <td class="fieldValue" id="{$MODULE}_detailView_fieldValue_{$FIELD_MODEL->getName()}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
					 ...
					 {if $FIELD_MODEL->isEditable() eq 'true' && ($FIELD_MODEL->getFieldDataType()!=Vtiger_Field_Model::REFERENCE_TYPE) && $IS_AJAX_ENABLED && $FIELD_MODEL->isAjaxEditable() eq 'true'}
						 {if $MODULE eq 'Campaigns' && ($FIELD_MODEL->get('name') eq "end_date" || $FIELD_MODEL->get('name') eq "closingdate" || $FIELD_MODEL->get('name') eq "start_time" || $FIELD_MODEL->get('name') eq "end_time" || $FIELD_MODEL->get('name') eq "targetaudience" || $FIELD_MODEL->get('name') eq "campaigntype" || $FIELD_MODEL->get('name') eq "sponsor" || $FIELD_MODEL->get('name') eq "targetsize" || $FIELD_MODEL->get('name') eq "actualsalescount" || $FIELD_MODEL->get('name') eq "expectedsalescount" || $FIELD_MODEL->get('name') eq "expectedresponsecount" || $FIELD_MODEL->get('name') eq "leaflet" || $FIELD_MODEL->get('name') eq "tag" || $FIELD_MODEL->get('name') eq "poster" || $FIELD_MODEL->get('name') eq "other1" || $FIELD_MODEL->get('name') eq "other2" || $FIELD_MODEL->get('name') eq "location" || $FIELD_MODEL->get('name') eq 'actual_send_to_hub' || $FIELD_MODEL->get('name') eq 'actual_tag' || $FIELD_MODEL->get('name') eq 'actual_checkup' || $FIELD_MODEL->get('name') eq 'actual_leaflet' || $FIELD_MODEL->get('name') eq 'actual_poster')}
                         {elseif $MODULE eq 'Campaigns' && $FIELD_MODEL->get('name') eq 'actualresponsecount' || $FIELD_MODEL->get('name') eq 'expectedresponse'}
                         {else}
