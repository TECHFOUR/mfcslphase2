...
{if $MODULE eq 'Targets' && $FIELD_MODEL->get("name") eq "revenue"}   
  <div class="input-prepend row-fluid">
	<div class="row-fluid date">
		<span class="add-on">{$USER_MODEL->get('currency_symbol')}</span>
		<input id="{$MODULE}_editView_fieldName_{$FIELD_NAME}" type="text" onkeyup="ValidatePriceField(this.id);" class="span9" style="margin-left:-1px;" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true} required,{/if}funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" name="{$FIELD_MODEL->getFieldName()}"
...
<div class="input-prepend row-fluid">
	<div class="row-fluid date">
		<span class="add-on">{$USER_MODEL->get('currency_symbol')}</span>
		<input id="{$MODULE}_editView_fieldName_{$FIELD_NAME}" type="text" class="span9" style="margin-left:-1px;" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true} required,{/if}funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" name="{$FIELD_MODEL->getFieldName()}"
		data-fieldinfo='{$FIELD_INFO}' value="{$FIELD_MODEL->getEditViewDisplayValue($FIELD_MODEL->get('fieldvalue'))}" {if !empty($SPECIAL_VALIDATOR)}data-validator={Zend_Json::encode($SPECIAL_VALIDATOR)}{/if} data-decimal-seperator='{$USER_MODEL->get('currency_decimal_separator')}' data-group-seperator='{$USER_MODEL->get('currency_grouping_separator')}' />
	</div>
</div>
{/if}
...
{else if ($FIELD_MODEL->get('uitype') eq '72') && ($FIELD_MODEL->getName() eq 'unit_price')}
	<div class="input-prepend">
		<div class="row-fluid">
			...
			<span class="span10 row-fluid" style="margin-left:-5px;">
				<input id="{$MODULE}-editview-fieldname-{$FIELD_NAME}" type="text" class="span11 unitPrice" name="{$FIELD_MODEL->getFieldName()}" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true} required,{/if}funcCall[Vtiger_Base_Validator_Js.invokeValidation]]"
