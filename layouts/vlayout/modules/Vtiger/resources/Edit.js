...
jQuery.Class("Vtiger_Edit_Js",{
	...
	registerEventStatusChangeEvent : function(container){
		...
		registerEventActivityTypeChangeEvent : function(container){
			...
			var assignoutletevent = container.find('.assginoutlet_div');
			var outletnameevent = container.find('.outletname_div');
		    ...
		    if(selectedOptiont == 'Appointment Booked'){
			   ...
			   assignoutletevent.show();
			}else{
			    driverPickupevent.hide();
				assignoutletevent.hide();
				outletnameevent.hide();
				$('#assigned_to_outlet').attr('checked',false);
				$('input[name="cf_951"]').val('');
				$('input[name="app_book_date"]').val('');
				$('select[name="cf_903"]').val('');
			     }
			});
			
			
			// Start code to show the Assign to Outlet when Activity Status will be 'Appintment Booked' and Sub Disposition will not 
			//be 'Appointment Cancelled' by jitendra singh on 25 Feb 2014 
			
			var assigned_to_outlet = container.find('.assigned_to_outlet');
			var Subdispostionevent = container.find('.cf_903');
			
			container.find('select[name="cf_903"]').on('change',function(e){
			var selectedOptiont = jQuery(e.currentTarget).val();// Jitendra Singh
		    if(selectedOptiont == 'Appointment Cancelled'){
				...
				$('#assigned_to_outlet').attr('checked',false);
				$('input[name="cf_951"]').val('');
				//$('input[name="cf_951"]').empty;
				}
			else{
				assignoutletevent.show();
				}	
			});
			var assigned_to_outlet_event = container.find('.assigned_to_outlet');

			container.find('input[name="assigned_to_outlet"]').on('change',function(e){
				
			if($('#assigned_to_outlet').attr('checked')) {
    			outletnameevent.show();
			} else {
    			outletnameevent.hide();
			}
			});
...
function AccessFormatedDate(val,dateFormat){   // Function defined for getting the formated data
 ...
 function ValidateDateForAppBook(val,id,dateformat,profile,campaign_type, activity_status){ // Appointment booking date *****Modified on 6 March 2014 by jitendra singh
...
if((campaign_type == 'Walk In' && profile == '5') || activity_status == 'Appointment Booked') { // Modified on 6 March 2014 by jitendra singh
...
/*Start code to validate Activity date when asssgining the lead from MO by jitendra singh on 19feb 2014*/
function ValidateleadAssignment(val,id,dateformat)  // This is for validaing the driver pickup - ISHWAR 24-dec-2013 
			  {   
			   	var yesterday = new Date(new Date().getTime() - 24 * 60 * 60 * 1000);
				var Caldate = new Date(AccessFormatedDate(val,dateformat));
				
				if(Caldate > yesterday){
					document.getElementById('leadactivitystartdate').value = val;
				} else {				
				alert('Activity Start Date should not be less than Current Date.');
				document.getElementById(id).value='';
				           }
		} 
		
/*End code to validate Activity date when asssgining the lead from MO by jitendra singh on 19feb 2014*/
