...
{if $MODULE eq 'Events' && $FIELD_MODEL->getFieldName() eq 'parent_id' }
...
<input id="{$FIELD_NAME}_display" name="{$FIELD_MODEL->getFieldName()}_display" type="hidden" class="{if $smarty.request.view eq 'Edit'} span7 {else} span8 {/if}	marginLeftZero autoComplete" {if !empty($displayId)}readonly="true"{/if}
...
{*<!--End code to hide related fields in acitvity form by jitendra singh -->*}	


{*<!--Start code to hide related fields in acitvity form by jitendra singh -->*}	
{else if ($MODULE eq 'Events' || $MODULE eq 'Calendar' ) && $FIELD_MODEL->getFieldName() eq 'cf_951' }

<input name="{$FIELD_MODEL->getFieldName()}" type="hidden" value="{$FIELD_MODEL->get('fieldvalue')}" class="sourceField" data-displayvalue='{$FIELD_MODEL->getEditViewDisplayValue($FIELD_MODEL->get('fieldvalue'))}' data-fieldinfo='{$FIELD_INFO}' />
{assign var="displayId" value=$FIELD_MODEL->get('fieldvalue')}


<div class="{if $FIELD_MODEL->get('fieldvalue') eq '' || $FIELD_MODEL->get('fieldvalue') eq '0' } hide {/if} outletname_div">
...
<span class="add-on clearReferenceSelection cursorPointer" 
	{if $MODULE eq 'Users' && $FIELD_NAME eq 'cf_775' && $IS_PREFERENCE eq true}style="display:none;"{/if} {*TECHFOUR*}
>
	<i id="{$MODULE}_editView_fieldName_{$FIELD_NAME}_clear" class='icon-remove-sign' title="{vtranslate('LBL_CLEAR', $MODULE)}"></i>
</span>

<input {if $MODULE eq 'Leads' && $FIELD_NAME eq 'make'} onfocus="getModelValues(this.id);" {/if}  
 id="{$FIELD_NAME}_display" name="{$FIELD_MODEL->getFieldName()}_display" type="text" class="{if $smarty.request.view eq 'Edit'} span7 {else} span8 {/if}	marginLeftZero autoComplete" {if !empty($displayId)}readonly="true"{/if}
 value="{$FIELD_MODEL->getEditViewDisplayValue($displayId)}" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true} required,{/if}funcCall[Vtiger_Base_Validator_Js.invokeValidation]]"
 data-fieldinfo='{$FIELD_INFO}' placeholder="{vtranslate('LBL_TYPE_SEARCH',$MODULE)}"
 {if !empty($SPECIAL_VALIDATOR)}data-validator='{Zend_Json::encode($SPECIAL_VALIDATOR)}'{/if}/>
