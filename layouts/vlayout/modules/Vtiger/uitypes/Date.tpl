...
{*<!--Modified on 6 March 2014 by jitendra singh -->*}
...
{elseif ($MODULE eq 'Events' || $MODULE eq 'Calendar' ) && ($FIELD_MODEL->get("name") eq "app_book_date") }
    ...
    <input readonly onchange="ValidateDateForAppBook(this.value,this.id,'{$dateFormat}','{$CURRENT_PROFILE}','{$CAMPAIGN_TYPE}','{$activity_status}');" id="{$MODULE}_editView_fieldName_{$FIELD_NAME}" type="text" style="width:190px;" class="span9 dateField" name="{$FIELD_MODEL->getFieldName()}" data-date-format="{$dateFormat}"
                type="text" value="{if $FIELD_MODEL->getEditViewDisplayValue($FIELD_MODEL->get('fieldvalue')) eq ''}{if $dateFormat eq 'yyyy-mm-dd'}{"tomorrow"|date_format:"%Y-%m-%d"}{/if}{if $dateFormat eq 'dd-mm-yyyy'}{"tomorrow"|date_format:"%d-%m-%Y"}{/if}{if $dateFormat eq 'mm-dd-yyyy'}{"tomorrow"|date_format:"%m-%d-%Y"}{/if}
   {else}
    	{if $FIELD_MODEL->getEditViewDisplayValue($FIELD_MODEL->get('fieldvalue')) eq '00-00-0000' || $FIELD_MODEL->getEditViewDisplayValue($FIELD_MODEL->get('fieldvalue')) eq '0000-00-00'}
