...
// Start code for duplicate check **********	
	...
	getRevenue : function(container){
		return jQuery('input[name="revenue"]',container).val();
	}, 
	getTarget : function(container){
		return jQuery('input[name="target"]',container).val();
	}, 
    ...
    registerAddingNewProductsAndServices: function(){
		...
		var lineItemTable = this.getLineItemContentsContainer();				
			...
			jQuery('#addProduct').on('click',function(){
				...
				var target_limit = $("#TARGETLIMITVALUE").val();
				if(target_limit == "")
					target_limit = 10;				
				if(total_active_Row < target_limit - 1) {										
					...
					alert("You are not allowed to create more than "+target_limit+ " targets at the same time.");	
				}		
...
/* Start added by ajay [TECHFOUR]*/			
	...
	registerRecordPreSaveEvent : function(form) {		
		...
		form.on(Vtiger_Edit_Js.recordPreSave, function(e, data) {															
			...
			var revenue = thisInstance.getRevenue(form);
			var target = thisInstance.getTarget(form);
			var end_date = thisInstance.getEndDate(form);			
			var assigned_to = thisInstance.getAssigned_To(form);
			var alltarget = 0;
			var allrevenue = 0;					
			//alert(total_sequence_Row);
			alltarget = target+"@"+assigned_to;
			allrevenue = revenue+"@"+assigned_to;
			for(var i = 1; i <= total_sequence_Row + 1; i++) {
				var deleted = $("#deleted"+i).val();				
				var salesperson = $("#salesperson"+i).val();	
				var target = $("#target"+i).val();
				var revenue = $("#revenue"+i).val();				
				if(salesperson != null && target != null && revenue != null) {					
					alltarget += "#"+target+"@"+salesperson;
					allrevenue += "#"+revenue+"@"+salesperson;														
				}
			}
			//alert(alltarget+"__"+allrevenue+"___"+i);	
			//alert(alltargetvalue);			
			var params = {};						
