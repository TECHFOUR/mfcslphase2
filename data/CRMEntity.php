...
// start added code by Ajay [TECHFOUR]
		if($cur_tab_id == $rel_tab_id && $cur_tab_id == 7) {
			
			$lead_qry = $adb->query("SELECT  mobile, smownerid FROM vtiger_leadaddress 								
			...
			if($adb->num_rows($lead_qry) > 0) {
				...
				$mobile = $lead_res['mobile'];
				$smownerid = $lead_res['smownerid'];
				$sub_query = " AND vtiger_users.id = ".$smownerid." ";  
				$userDetails = getUserDetails($sub_query);				
				$outletmasterid = $userDetails['outletmasterid'];
										
			}
			
			$rel_lead_qry = $adb->query("SELECT  vtiger_leadaddress.leadaddressid as lead_id FROM vtiger_leadaddress 								
								INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_leadaddress.leadaddressid
								INNER JOIN vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid
								INNER JOIN vtiger_outletmastercf on vtiger_outletmastercf.outletmasterid = vtiger_users.cf_775		
								WHERE vtiger_crmentity.deleted = 0 AND mobile = $mobile AND outletmasterid = $outletmasterid AND vtiger_crmentity.crmid <> $id ");	
			if($adb->num_rows($rel_lead_qry) > 0) {				
				$all_rel_lead = "";
				$count = 0;
				while($row = $adb->fetch_array($rel_lead_qry)) {
					$lead_id = $row['lead_id'];
					if($count == 0)
						$all_rel_lead = $lead_id;
					if($count > 0 )
						$all_rel_lead .= ', '.$lead_id;	
					$count++;										
				}
			...
			$query .= " WHERE vtiger_crmentity.deleted = 0 AND vtiger_crmentityrel.crmid in($all_rel_lead) ";
		}
		else
...
//echo "<pre>";print_r($return_value);die;
		if ($return_value == null)
			$return_value = Array();
		$return_value['CUSTOM_BUTTON'] = $button;
		return $return_value;
	...
	function getNonAdminAccessControlQuery($module, $user, $scope = '') {
		...
		// Start added by AJAY [TECHFOUR]
			if($module == "Leads" && $_REQUEST['relatedModule'] == $module)
				$query = "";
