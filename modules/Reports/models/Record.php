...
class Reports_Record_Model extends Vtiger_Record_Model {
	...
	function getAdvancedFilterSQL() {				
		$summary_val = $this->get('summary_val');
		$region_name = $this->get('region_name');
		$outlet_id = $this->get('outlet_id');
		$agent_id = $this->get('agent_id');
		
		if($region_name != "")
			$filterQuery .= " AND vtiger_outletmastercf.cf_781 = '$region_name' ";
		
		if($outlet_id != "")
			$filterQuery .= " AND vtiger_outletmastercf.outlet = '$outlet_id' ";
			
		if($agent_id != "" )
			$filterQuery .= " AND vtiger_crmentity.smownerid = $agent_id ";
					
		/*
		$advancedCustomFilter = $this->get('advancedCustomFilter');	
		foreach($advancedCustomFilter as $key => $row) {
			if($row != "") {
				if($key == 'agent_id' && $summary_val == 0)
					$filterQuery .= " AND vtiger_crmentity.smownerid = $row ";					
