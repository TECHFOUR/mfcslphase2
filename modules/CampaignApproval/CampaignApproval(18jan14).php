...
/**   Function used to send email
  ...
  */
  ...
  if($currentid != "" && $sourceRecord != "") {
	  	...
	  	$campaign_qry = $adb->pquery("select smownerid, location, end_date, closingdate,  vtiger_campaign.campaignid as campid, campaignname, targetsize, email3, email4, email6 from vtiger_campaign 
		...
		where vtiger_campaign.campaignid = ?", array($sourceRecord));		
  		if($adb->num_rows($campaign_qry) > 0) {
			...
			$report_email4 = $adb->query_result($campaign_qry,0,'email4');
			$report_email6 = $adb->query_result($campaign_qry,0,'email6');
...
where vtiger_crmentity.deleted = 0 and vtiger_campaign.campaignid = ".$sourceRecord." ");	
	...
	$role_Manager = "";	
		...
		switch($depth1) {			
			case 2:
			$depth_next = 0;			
			$allemails =  $report_email6.', '.$report_email4.', '.$report_email3;					
			break;
												
			case 3:
			$depth_next = 2;
			$role_Manager = "Head Business Development";									
			$allemails =  $report_email6.', '.$report_email4;
			break;
			
			case 4:
			$depth_next = 3;			
			$allemails =  $report_email6;
			$role_Manager = "BD Corporate";
			break;
			
			default:			
			$depth_next = 4;			
			$role_Manager = "Reagional Manager ".$zone1;
		}
...
$descriptionsalesapproved = str_replace("[RemarksData]",$remarksdata,$descriptionsalesapproved);
		...
		if($approvalstatus == "Approve") {
			...
			if($permission_up_report == 1) {
				...
				$adb->query("UPDATE vtiger_campaign 
				INNER JOIN vtiger_campaignscf ON vtiger_campaignscf.campaignid = vtiger_campaign.campaignid 
				SET approveddate = '".$approveddate."', campaignstatus = 'Approved', approval_stage = '' , emailstatus6 = '1', emailstatus4 = '1', emailstatus3 = '1', emailstatus2 = '1' where vtiger_campaign.campaignid = ".$sourceRecord." ");
			...
			$adb->query("UPDATE vtiger_campaign 
			INNER JOIN vtiger_campaignscf ON vtiger_campaignscf.campaignid = vtiger_campaign.campaignid 
			SET campaignstatus = 'Rejected', approval_stage = ".$smownerid." , emailstatus6 = '0', emailstatus4 = '1', emailstatus3 = '1', emailstatus2 = '1',  ".$email_field1." = '".$email1."' where vtiger_campaign.campaignid = ".$sourceRecord." ");
			$descriptions = $descriptionsalesapproved;
			if($permission_up_report == 1) {
				$email2 = $email1;
			}			
