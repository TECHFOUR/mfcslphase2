...
//			}
	...
	public static function showResult($importInfo, $importStatusCount) {
		...
		if($moduleName == 'Leads'){
			$total_records = $_SESSION['total_records']-1;
			$file_path = $_SESSION['log_error_path'];
			$updated_records = $_SESSION['updated_records'];
			$skipped_records = $total_records - $importStatusCount['TOTAL'];
			unset($_SESSION['total_records']);
		...
		// Code Added by jitendra singh on 24 Dec 2013
		if($moduleName == 'Leads'){
			$viewer->assign('total_records',$total_records);
