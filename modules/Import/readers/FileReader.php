...
class Import_FileReader_Reader {
	...
	public function addRecordToDB($columnNames, $fieldValues, $i,$row,$filename,$handle) {
		...
		if($_REQUEST['module'] == 'Leads'){
			...
			$logerror = "";
			$fileter_mobile = preg_replace( '/[^0-9]/', '', $fieldValues[0]);
			
		if($fieldValues[0] == '' || strlen($fileter_mobile) != 10 || ($fieldValues[1] == '' && $fieldValues[2] == '')){ // Start code when skip lead
