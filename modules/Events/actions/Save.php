		...
		if($startTime != '' && $startDate != '' && ($followupMode == 'on') ) {
				...
				if($appointmentbook == "Appointment Booked") {					
					...
					if($appointmentbook != "Call Back") {
						...
						$recordModel->set('driver_pickup',$driver_flag);					
					}
					$adb->query("UPDATE vtiger_leaddetails INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_leaddetails.leadid 
						...
						SET app_book_date = '".$new_date_start."', app_book_time = '".$new_startTime."', leadstatus = 'Appointment Booked', modifiedby = ".$current_user->id.", modifiedtime = '".date('Y-m-d H:i:s')."' , latest_activity_date = '".$startDate."' 
						WHERE vtiger_leaddetails.leadid = ".$_REQUEST['parent_id']." ");
				}else {
					$recordModel->set('date_start',$startDate);
					$recordModel->set('due_date',$startDate);
					$recordModel->set('time_start',$startTime);
					$recordModel->set('time_end',$startTime);
					$recordModel->set('app_book_date','');
                	$recordModel->set('app_book_time','');
					$recordModel->set('cf_903','');
					$recordModel->set('driver_pickup',0);
					$adb->query("UPDATE vtiger_leaddetails INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_leaddetails.leadid 
						INNER JOIN vtiger_leadscf ON vtiger_leadscf.leadid = vtiger_leaddetails.leadid
						SET app_book_date = '', app_book_time = '', leadstatus = 'Open', modifiedby = ".$current_user->id.", modifiedtime = '".date('Y-m-d H:i:s')."' , latest_activity_date = '".$startDate."' 
						WHERE vtiger_leaddetails.leadid = ".$_REQUEST['parent_id']." ");
				}
