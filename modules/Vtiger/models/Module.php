...
// start added code by ajay 
			if($profileid == 5 ) // CRO
				 $query .= " AND tabid IN(3, 7, 9, 52, 25)";
			else if($profileid == 6 )// Agent
				 $query .= " AND tabid IN(3, 7, 9, 25)";
			else if($profileid == 4 )// MO
				 $query .= " AND tabid IN(3, 7, 9, 26, 47, 50, 33, 53, 25)";
			else if($profileid == 3)// Agent Central
				 $query .= " AND tabid IN(3, 7, 9, 25)";
			else if($profileid == 12 || $profileid == 13)// Superviser Central
				 $query .= " AND tabid IN(3, 7, 9, 53, 26, 25)";	
					
// end added code by ajay 
            $result = $db->pquery($query, $params);
...
//echo "<pre>";print_r($moduleModels);die;
		return $moduleModels;
	}
  ...
  count(distinct case when (DATEDIFF('".$newcurrentdate."', date_start) = 0) AND vtiger_activity.eventstatus = 'Held' then vtiger_activity.activityid end) AS 'DDDHeld',
   vtiger_crmentity.crmid, vtiger_crmentity.smownerid, vtiger_crmentity.setype, vtiger_activity.* ";
			...
			$query_select = " SELECT vtiger_crmentity.crmid, crmentity2.crmid AS parent_id, vtiger_crmentity.smownerid, vtiger_crmentity.setype, vtiger_activity.* ";
		...
		if($depthdashboard_permission == 1)	{ // For MO		
...
//			);
	...
	public function getSummaryViewFieldsList() {
		//echo "<pre>";print_r($this->summaryFields);
