...
// Start added code by Ajay [TECHFOUR]
		if($this->getModuleName() == "Campaigns")
			$permissionvalue = $this->getEditPermission($module, $this->getId());
...
// Start added code by Ajay [TECHFOUR]
	function getEditPermission($module, $id) {		
		global $adb, $current_user;	
		$sub_query = " AND vtiger_users.id = ".$current_user->id." ";
		$userDetails = getUserDetails($sub_query);
		if($userDetails['depth'] == 6) {
			$campaign_qry = $adb->query("SELECT draft_campaign from vtiger_campaignscf where campaignid = ".$id." ");
			if($adb->num_rows($campaign_qry) > 0) {
				$campaign_res = $adb->fetch_array($campaign_qry);
				$draft_campaign = $campaign_res['draft_campaign'];
