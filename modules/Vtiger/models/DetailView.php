...
// Start added by Ajay [TECHFOUR]
		$edit_permission = 0;
		if(($moduleName == "Targets" &&  $userid != 1)) {
			$sub_query = " AND vtiger_users.id = ".$userid." ";		
			$userDetails = getUserDetails($sub_query);
			$new_depth = $userDetails['depth'];
			if($new_depth == 5) { // For RBDM Role				
				$crm_qry = $adb->query("SELECT smownerid from vtiger_crmentity where deleted = 0 AND crmid = ".$recordId." ");
				if($adb->num_rows($crm_qry) > 0) {
					$res_qry = $adb->fetch_array($crm_qry);
					if($res_qry['smownerid'] == $userid)				
						$edit_permission = 1;
				}
			}
		}
// End added by Ajay [TECHFOUR]	
		...
		if(Users_Privileges_Model::isPermitted($moduleName, 'EditView', $recordId) && $edit_permission == 0) {
		...
		if(Users_Privileges_Model::isPermitted($moduleName, 'Delete', $recordId) && $edit_permission == 0) {
