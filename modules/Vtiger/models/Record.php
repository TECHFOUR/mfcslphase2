...
// Start added code by Ajay [TECHFOUR]
		if($modulename == "Campaigns" || $modulename == "Targets")
			$permissionvalue = $this->getEditPermission($modulename, $this->getId());					
...
// Start added code by Ajay [TECHFOUR]
	function getEditPermission($modulename, $id) {		
		global $adb, $current_user;	
		$sub_query = " AND vtiger_users.id = ".$current_user->id." ";
		$userDetails = getUserDetails($sub_query);
		
		if($userDetails['depth'] == 8 && $modulename == "Campaigns") { // For MO Role
			$campaign_qry = $adb->query("SELECT 1 from vtiger_campaignscf 
                ...
                where deleted = 0 AND vtiger_campaignscf.campaignid = ".$id." ");
			if($adb->num_rows($campaign_qry) > 0) {				
				return 1;
			}
		}
		
		else if($userDetails['depth'] == 5 && $modulename == "Targets") { // For RBDM Role
			$crm_qry = $adb->query("SELECT smownerid from vtiger_crmentity where deleted = 0 AND crmid = ".$id." ");
			if($adb->num_rows($crm_qry) > 0) {
				$res_qry = $adb->fetch_array($crm_qry);
				if($res_qry['smownerid'] == $current_user->id)				
					return 1;
			}
		}
...
// End added code by Ajay [TECHFOUR]	
	 ...
	 * @return <String>
	 */
	public function getModuleDashboardValues($recordId = false, $fieldName = false) {
		global $adb;
		$tempfieldName = "";
		if($fieldName == "fullname") {
			$tempfieldName = "fullname";
			$firstname = "firstname";
			$lastname = "lastname";
			$fieldName = 'CONCAT('.$firstname.'," ",'.$lastname.') AS fullname';		
		}
		
		$lead_activity_qry = $adb->query("SELECT $fieldName  FROM vtiger_seactivityrel 
								...
								WHERE  vtiger_activity.activityid = ".$recordId."  ");
		if($tempfieldName != "")
			$fieldName = "fullname";	
