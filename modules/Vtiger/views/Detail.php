...
class Vtiger_Detail_View extends Vtiger_Index_View {
	...
	function preProcess(Vtiger_Request $request, $display=true) {
		...
		global $adb;
		global $current_user;
		$current_user_id = $current_user->isadmin; // Find the current user is admin or not by jitendra singh on 15 feb 14
		...
		$viewer->assign('IS_ADMIN', $current_user_id);  // Assing the current user is admin or not by jitendra singh on 15 feb 14

		$linkParams = array('MODULE'=>$moduleName, 'ACTION'=>$request->get('view'));
		$linkModels = $this->record->getSideBarLinks($linkParams);
		if($_SESSION['authenticated_user_id'] == 1) // added by ajay
	...
	function process(Vtiger_Request $request) {
		...
		global $adb;
		// Start code to find Customer type (If Individual then Add Activity button will not show in Summary of Leads) by jitendra singh on 5 March 2014
		...
		$viewer->assign('MAIN_MODULE', $_REQUEST['module']);
		// End code to find Customer type (If Individual then Add Activity button will not show in Summary of Leads) by jitendra singh on 5 March 2014
...
// Start added code by Ajay [TECHFOUR]
		if($moduleName == "CampaignApproval" && $_SESSION['authenticated_user_id'] != 1) {
			$viewer->assign('IS_AJAX_ENABLED', 0);
		}
		
		if($moduleName == "Targets" && $_SESSION['authenticated_user_id'] != 1) {
			$target_query = $adb->query("select salesperson from vtiger_targetscf 		
			INNER JOIN vtiger_crmentity on vtiger_crmentity.crmid = vtiger_targetscf.targetsid 						
			where vtiger_crmentity.deleted = 0 and targetsid = ".$recordId." ");							
			if($adb->num_rows($target_query) > 0) {
				$target_res = $adb->fetch_array($target_query);
				$assignedto = $target_res['salesperson'];
				$sub_query = " AND vtiger_users.id = ".$assignedto." ";		
...
// Start for Campaign ****************
	if($moduleName == "Campaigns") {
		$sub_query = " AND vtiger_users.id = ".$_SESSION['authenticated_user_id']." ";
		$userDetails = getUserDetails($sub_query);
		if($userDetails['depth'] == 8) {
			$viewer->assign('IS_AJAX_ENABLED', $this->checkEditPermission($recordId));			
		}
		else
			$viewer->assign('IS_AJAX_ENABLED', $this->isAjaxEnabled($recordModel));
	}
...
// End for Campaign ****************			
	...
	function checkEditPermission($recordId) {
		global $adb;			
			$campaign_qry = $adb->query("SELECT 1 from vtiger_campaignscf 
                INNER JOIN vtiger_leadaddress ON vtiger_leadaddress.campaignid = vtiger_campaignscf.campaignid 
                INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_leadaddress.leadaddressid
                where deleted = 0 AND vtiger_campaignscf.campaignid = ".$recordId." ");
