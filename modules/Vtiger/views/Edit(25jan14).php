...
Class Vtiger_Edit_View extends Vtiger_Index_View {
	...
	public function process(Vtiger_Request $request) {
		...
		if($record != '' && $moduleName == 'Campaigns'){
			...
			$target_query = $adb->query("select draft_campaign from vtiger_campaignscf where campaignid = ".$record." ");
			if($adb->num_rows($target_query) > 0) {
				$resultnum = $adb->fetch_array($target_query);
				$draft_campaign = $resultnum['draft_campaign'];
			}
			/*End*/
		...
		if($_REQUEST['record'] != '' && $moduleName == 'Campaigns'){
			$viewer->assign('DRAFT_CAMPAIGNS', $draft_campaign);
