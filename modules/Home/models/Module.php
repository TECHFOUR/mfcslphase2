...
class Home_Module_Model extends Vtiger_Module_Model {
	...
	function getCalendarActivities($mode, $pagingModel, $user) {
		...
		$profile_permission = $userDetails['profile_permission'];
		$depthdashboard_permission = 1;
		
		if($profile_permission == 1) {	// Region Level			
			$groupby = " GROUP BY vtiger_outletmastercf.cf_781 ";
			$select_firstcol = " vtiger_outletmastercf.cf_781 ";
		}
		elseif($profile_permission == 2) { // Manager level			
			$groupby = " GROUP BY vtiger_outletmaster.outletmasterid ";
			$select_firstcol = " vtiger_outletmastercf.outlet ";		
		}
		elseif($profile_permission == 3) { // Mo level				
			$groupby = " GROUP BY smownerid ";
			$select_firstcol = "CASE WHEN(vtiger_users.last_name NOT LIKE '' AND vtiger_crmentity.crmid != '') THEN CONCAT(vtiger_users.first_name,' ',vtiger_users.last_name) ELSE vtiger_groups.groupname END ";
		}
		
		else// Agent and admin level
			$depthdashboard_permission = 0;								
			
		if($depthdashboard_permission == 1) {
			$query_select = " SELECT $select_firstcol AS 'Agent' ,
count(distinct case when DATEDIFF('".$newcurrentdate."', date_start) > 3 then vtiger_activity.activityid end) AS 'totalPDPastCall',
  ...
  count(distinct case when (DATEDIFF('".$newcurrentdate."', DATE(vtiger_crmentity.modifiedtime)) = 0) AND vtiger_activity.eventstatus = 'Held' then vtiger_activity.activityid end) AS 'DDDHeld',
   vtiger_crmentity.crmid, vtiger_crmentity.smownerid, vtiger_crmentity.setype, vtiger_activity.* ";
		...
		$query = " $query_select FROM vtiger_activity
					...
					//INNER JOIN vtiger_crmentity AS leadcrm ON(leadcrm.crmid = vtiger_leaddetails.leadid AND leadcrm.deleted=0) 

		$query .= Users_Privileges_Model::getNonAdminAccessControlQuery('Calendar');
				
		$query .= " WHERE vtiger_crmentity.deleted=0
					AND vtiger_activitycf.re_assigned_to = 0
