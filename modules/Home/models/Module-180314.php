...
class Home_Module_Model extends Vtiger_Module_Model {
	 ...
	 */
	 ...
	 /*SELECT DATE(modifiedtime), CASE WHEN(vtiger_users.last_name NOT LIKE '' AND vtiger_crmentity.crmid != '') THEN CONCAT(vtiger_users.first_name,' ',vtiger_users.last_name) ELSE vtiger_groups.groupname END AS 'Agent', count(distinct case when DATEDIFF('2014-03-03', DATE(modifiedtime)) = 3 then vtiger_activity.activityid end) AS 'PD', count(distinct case when (DATEDIFF('2014-03-03', DATE(modifiedtime)) = 3) AND vtiger_activity.eventstatus = 'Held' then vtiger_activity.activityid end) AS 'PDHeld', count(distinct case when DATEDIFF('2014-03-03', DATE(modifiedtime)) = 2 then vtiger_activity.activityid end) AS 'D', count(distinct case when (DATEDIFF('2014-03-03', DATE(modifiedtime)) = 2) AND vtiger_activity.eventstatus = 'Held' then vtiger_activity.activityid end) AS 'DHeld', count(distinct case when DATEDIFF('2014-03-03', DATE(modifiedtime)) = 1 then vtiger_activity.activityid end) AS 'DD', count(distinct case when (DATEDIFF('2014-03-03', DATE(modifiedtime)) = 1) AND vtiger_activity.eventstatus = 'Held' then vtiger_activity.activityid end) AS 'DDHeld', count(distinct case when DATEDIFF('2014-03-03', DATE(modifiedtime)) = 0 then vtiger_activity.activityid end) AS 'DDD', count(distinct case when (DATEDIFF('2014-03-03', DATE(modifiedtime)) = 0) AND vtiger_activity.eventstatus = 'Held' then vtiger_activity.activityid end) AS 'DDDHeld', vtiger_crmentity.crmid, vtiger_crmentity.smownerid, vtiger_crmentity.setype, vtiger_activity.* 

FROM vtiger_activity INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_activity.activityid INNER JOIN vtiger_activitycf ON vtiger_activitycf.activityid = vtiger_activity.activityid LEFT JOIN vtiger_groups ON vtiger_groups.groupid = vtiger_crmentity.smownerid LEFT JOIN vtiger_users ON vtiger_users.id = vtiger_crmentity.smownerid 
 WHERE vtiger_crmentity.deleted=0 AND (vtiger_activity.activitytype NOT IN ('Emails')) AND (vtiger_activity.status is NULL OR vtiger_activity.status NOT IN ('Completed', 'Deferred')) AND DATE(modifiedtime) between '2014-02-28' AND '2014-03-03' group BY smownerid*/
	function getCalendarActivities($mode, $pagingModel, $user) {
		...
		if($PROFILEID == 2 ||  $PROFILEID == 4 ||  $PROFILEID == 7 ||  $PROFILEID == 8 ||  $PROFILEID == 9 ||  $PROFILEID == 10 || $PROFILEID == 11 || $PROFILEID == 12 || $PROFILEID == 13)	
  ...
  count(distinct case when (DATEDIFF('".$newcurrentdate."', DATE(vtiger_crmentity.modifiedtime)) = 3) AND vtiger_activity.eventstatus = 'Held' then vtiger_activity.activityid end) AS 'PDHeld',    
  count(distinct case when DATEDIFF('".$newcurrentdate."', date_start) = 2 then vtiger_activity.activityid end) AS 'D',
  count(distinct case when (DATEDIFF('".$newcurrentdate."', DATE(vtiger_crmentity.modifiedtime)) = 2) AND vtiger_activity.eventstatus = 'Held' then vtiger_activity.activityid end) AS 'DHeld', 
  count(distinct case when DATEDIFF('".$newcurrentdate."', date_start) = 1 then vtiger_activity.activityid end) AS 'DD',
  count(distinct case when (DATEDIFF('".$newcurrentdate."', DATE(vtiger_crmentity.modifiedtime)) = 1) AND vtiger_activity.eventstatus = 'Held' then vtiger_activity.activityid end) AS 'DDHeld', 
  count(distinct case when DATEDIFF('".$newcurrentdate."', date_start) = 0 then vtiger_activity.activityid end) AS 'DDD',
  count(distinct case when (DATEDIFF('".$newcurrentdate."', DATE(vtiger_crmentity.modifiedtime)) = 0) AND vtiger_activity.eventstatus = 'Held' then vtiger_activity.activityid end) AS 'DDDHeld',
   vtiger_crmentity.crmid, vtiger_crmentity.smownerid, vtiger_crmentity.setype, vtiger_activity.* ";
		...
		if($depthdashboard_permission == 1)	{ // Start for Mo only	
			$query .= " AND ((modifiedtime between  '$newpastdate' AND '$newcurrentdate') OR  (date_start between  '$newpastdate' AND '$newcurrentdate')) ";	
			...
			elseif ($mode === 'overdue') // // Followup Calling
				$query .= " AND cf_895 = 'Call Back'";
			/*elseif ($mode === 'today') { // Today Calling
				$query .= " AND ((modifiedtime between  '$newpastdate' AND '$newcurrentdate') OR  (date_start between  '$newpastdate' AND '$newcurrentdate'))  ";
				//AND cf_895 NOT IN('Appointment Booked', 'Call Back')				
			}*/
		} // End for Mo only	
		else {
			if ($mode === 'upcoming') { // Appointment booked
			...
			elseif ($mode === 'today') { // Today New Calling
				$query .= " AND date_start <= '$currentDate' ";
				$query .= "  AND cf_895 NOT IN('Appointment Booked', 'Call Back')";	
			}
		}
