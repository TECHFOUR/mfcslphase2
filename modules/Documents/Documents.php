...
if(isset($_REQUEST['currentid']) && $_REQUEST['sourceRecord'] != '') {	  // Modified by jitendra[TECHFOUR] on 9 Nov 2013

	for($i=1; $i<=2; $i++) {
		$title = $_REQUEST['notes_title'.$i];
		$note = mysql_real_escape_string($_REQUEST['notecontent'.$i]);
		...
		if($delete == 0 && $title != '' && $filename != '') {
		  ...
		  $this->insertintonotesrel($_REQUEST['sourceRecord'],$crmid);
