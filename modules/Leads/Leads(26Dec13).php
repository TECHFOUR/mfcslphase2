	...
	function save_module($module)
	{
		...
		if($_REQUEST['mode'] != "import"){
				...
				if(isset($_REQUEST['currentid']) && $_REQUEST['currentid'] != ""){			
					$adb->query("UPDATE vtiger_leaddetails SET import_lead_flag = '0' where leadid = ".$_REQUEST['currentid']." ");
				}
				
				if(!strstr($rolename,'Central')) {
					$adb->query("UPDATE vtiger_leaddetails SET outlet = ".$outletmasterid." where leadid = ".$this->id." ");
				}
		
				$campaignid = $_REQUEST['campaignid'];
				$campaign_qry = $adb->pquery("select campaigntype, location from vtiger_campaign INNER JOIN vtiger_campaignscf on vtiger_campaignscf.campaignid = vtiger_campaign.campaignid where vtiger_campaign.campaignid = ?", array($campaignid));		
		...
		$campaign_qry = $adb->pquery("select campaigntype, location,vtiger_campaignscf.campaignid as 'campid' from vtiger_campaign INNER JOIN vtiger_campaignscf on vtiger_campaignscf.campaignid = vtiger_campaign.campaignid where vtiger_campaign.campaignid = ?", array($importid));		
  		if($adb->num_rows($campaign_qry) > 0) {
			$campaigntype = $adb->query_result($campaign_qry,0,'campaigntype');
			$campaignlocation = $adb->query_result($campaign_qry,0,'location');
			$campid = $adb->query_result($campaign_qry,0,'campid');		
			$adb->pquery("update vtiger_leadaddress inner join vtiger_leadsubdetails on vtiger_leadsubdetails.leadsubscriptionid = vtiger_leadaddress.leadaddressid set campaigntype = ?, campaignlocation = ?, campaignid = ? where leadaddressid=? ",array($campaigntype,$campaignlocation,$campid,$this->id));			
		}
		
				$sub_query = " AND vtiger_users.id = ".$import_user." ";
				$userDetails = getUserDetails($sub_query);
				$depth = $userDetails['depth'];
				$rolename = $userDetails['rolename'];
				$outletmasterid = $userDetails['outletmasterid'];
		...
		if($this->mode == "edit" && $_REQUEST['record'] != "") {$history_qry = $adb->query("SELECT lead_no, campaignid, vtiger_modtracker_basic.id as basicid, vtiger_modtracker_basic.module as basicmodule, vtiger_modtracker_basic.crmid as entityid, whodid, createdtime, changedon, prevalue, postvalue, outletmaster FROM vtiger_modtracker_basic 
...
WHERE vtiger_crmentity.deleted = 0 AND vtiger_modtracker_detail.fieldname = 'assigned_user_id' AND history_status = '0' AND vtiger_modtracker_basic.module = 'Leads' ORDER BY basicid");		
	...
	// Start to add activity ************************************************************
		if($this->mode != "edit" && $this->column_fields['leadsource'] == "Individual") {
			...
			$query = "INSERT INTO vtiger_crmentity (crmid,smcreatorid,smownerid,modifiedby,setype,description,createdtime,
						modifiedtime,viewedtime,status,version,presence,deleted,label) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
			$adb->pquery($query, array($crmid, $current_user->id, $assignedid, $current_user->id, "Calendar", "", $currentdatetime, $currentdatetime, NULL, NULL, 0, 1, 0, $callerName));
			...
			$adb->query("INSERT into vtiger_activitycf (activityid) values(".$crmid.")"); 
			...
			$adb->query("INSERT into vtiger_seactivityrel (crmid,activityid) values(".$this->id.",".$crmid.")");	
...
WHERE vtiger_crmentity.deleted = 0 AND vtiger_leaddetails.leadid = $this->id " );		
