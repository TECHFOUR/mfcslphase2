...
class Leads_Module_Model extends Vtiger_Module_Model {
    ...
    */
	...
	public function getRecentRecords($limit=10) {
		...
		$query = 'SELECT * FROM vtiger_crmentity '.
            ' INNER JOIN vtiger_leaddetails ON
                ...
                WHERE setype=? AND '.$deletedCondition.' AND modifiedby = ? ORDER BY createdtime ASC LIMIT ?';
	 ...
	 * @return <Array>
	 */
	public function getAssignDriver($pagingModel) {		
		$currentUser = Users_Record_Model::getCurrentUserModel();
		$db = PearDatabase::getInstance();
		$currentDate = date('Y-m-d');
		$newpastdate = date('Y-m-d',strtotime('- 1 days', strtotime($currentDate)));		
		$query = "SELECT vtiger_leaddetails.leadid AS leadid, firstname, lastname, mobile, driverid, drivermaster FROM vtiger_leaddetails
...
INNER JOIN vtiger_crmentity AS crmentity_lead ON crmentity_lead.crmid = vtiger_leaddetails.leadid
LEFT JOIN vtiger_crmentityrel ON (vtiger_crmentityrel.crmid = vtiger_leaddetails.leadid AND vtiger_crmentityrel.relmodule = 'Driver')


LEFT JOIN vtiger_drivercf ON (vtiger_drivercf.driverid = vtiger_crmentityrel.relcrmid AND vtiger_crmentityrel.relmodule = 'Driver')
LEFT JOIN vtiger_crmentity AS crmentity ON (crmentity.crmid = vtiger_drivercf.driverid AND crmentity.deleted = 0)
LEFT JOIN vtiger_drivermaster ON vtiger_drivermaster.drivermasterid = vtiger_drivercf.cf_877
LEFT JOIN vtiger_crmentity AS crm_dr_master ON (crm_dr_master.crmid = vtiger_drivermaster.drivermasterid AND crm_dr_master.deleted = 0)
 ".Users_Privileges_Model::getNonAdminAccessControlQuery($this->getName())."
 WHERE vtiger_crmentity.deleted = 0 AND crmentity_lead.deleted = 0 AND driver_pickup = 1 AND cf_903 NOT IN('Appointment Cancelled') AND leadstatus = 'Appointment Booked' 
 AND app_book_date between '".$newpastdate."' AND '".$currentDate."'
GROUP BY vtiger_leaddetails.lead_no ORDER BY vtiger_seactivityrel.activityid DESC ";
					
//LIMIT ".$pagingModel->getStartIndex().", ".$pagingModel->getPageLimit()."						
		$result = $db->pquery($query, array());
		$models = array();
		for($i=0; $i<$db->num_rows($result); $i++) {
			$modelInstance = Vtiger_Record_Model::getCleanInstance('Leads');
			$modelInstance->setId($db->query_result($result, $i, 'leadid'));
			$modelInstance->set('leadid', $db->query_result($result, $i, 'leadid'));
			$modelInstance->set('firstname', $db->query_result($result, $i, 'firstname'));
			$modelInstance->set('lastname', $db->query_result($result, $i, 'lastname'));
			$modelInstance->set('mobile', $db->query_result($result, $i, 'mobile'));
			$modelInstance->set('driverid', $db->query_result($result, $i, 'driverid'));
			$modelInstance->set('fullname', $db->query_result($result, $i, 'drivermaster'));
			$models[] = $modelInstance;
		}
		return $models;
	}

	public function getDriverLetter($pagingModel) {		
		$currentUser = Users_Record_Model::getCurrentUserModel();
		$db = PearDatabase::getInstance();		
		$query = "SELECT leadid, lead_no, drivermaster, cf_879, driverid
FROM vtiger_leaddetails
INNER JOIN vtiger_crmentityrel ON vtiger_crmentityrel.crmid = vtiger_leaddetails.leadid
INNER JOIN vtiger_drivercf ON (vtiger_drivercf.driverid = vtiger_crmentityrel.relcrmid AND vtiger_crmentityrel.relmodule = 'Driver')
INNER JOIN vtiger_drivermaster ON vtiger_drivermaster.drivermasterid = vtiger_drivercf.cf_877
INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_leaddetails.leadid
INNER JOIN vtiger_crmentity AS crmentity ON crmentity.crmid = vtiger_drivercf.driverid
 ".Users_Privileges_Model::getNonAdminAccessControlQuery($this->getName())."
WHERE vtiger_crmentity.deleted = 0 AND crmentity.deleted = 0 ";
		$result = $db->pquery($query, array());
		$models = array();
		for($i=0; $i<$db->num_rows($result); $i++) {
			$modelInstance = Vtiger_Record_Model::getCleanInstance('Leads');
			$modelInstance->setId($db->query_result($result, $i, 'leadid'));
			$modelInstance->set('lead_no', $db->query_result($result, $i, 'lead_no'));
			$modelInstance->set('fullname', $db->query_result($result, $i, 'drivermaster'));			
			//$modelInstance->set('driver_mobile', $db->query_result($result, $i, 'cf_879'));
			$modelInstance->set('driverid', $db->query_result($result, $i, 'driverid'));
			$models[] = $modelInstance;
		}
