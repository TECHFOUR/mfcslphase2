	...
	function save_module($module)
	{
		...
		//if($_REQUEST['mode'] != "import"){
			...
			if($currentprofileid == 3 || $currentprofileid == 6 || $currentprofileid == 5) {
				...
				if(($mo_profileid == 4 || $mo_profileid == 13) && ($current_user->id != $assignedid)) { // MO & Superviser profile Reassignment							
					$this->createNewActivity($current_user->id, $assignedid, $this->id, $_REQUEST['leadactivitystartdate']);
				}
				...
				$campaign_qry = $adb->pquery("select campaigntype, location, vendorname, 
				...
				if($adb->num_rows($campaign_qry) > 0) {
					...
					$campaign_category = $adb->query_result($campaign_qry,0,'campcategory');					
					$adb->pquery("update vtiger_leadaddress 
					...
					set campaigntype = ?, campaignlocation = ?, campaign_category = ? where leadaddressid=? ",array($campaigntype,$campaignlocation, $campaign_category, $this->id));			
				}
				...
				// start to add except inbound lead will transfer to MO if created by agent or cro ***************
					if($campaign_category != "InBound" && ($profileid == 5 || $profileid == 6) && isset($_REQUEST['currentid'])) {
...
WHERE vtiger_crmentity.deleted = 0 AND vtiger_modtracker_detail.fieldname = 'assigned_user_id' AND history_status = '0' AND vtiger_modtracker_basic.module = 'Leads' ORDER BY basicid");		
	...
	// Start to add activity ************************************************************
		if($this->mode != "edit" && $customer_type != "Corporate" && $campaign_category == "InBound") {
