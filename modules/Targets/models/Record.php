...
class Targets_Record_Model extends Vtiger_Record_Model {
	...
	public function checkDuplicate() {							
		...
		if(count($usersid) != count(array_unique($usersid)))
			return "outlet_level### There is repeated Target allocation for the same users.";
		...
		if(is_array($all_old_users)) {		
			...
			if((!in_array($userid, $all_old_users)) && $record == "" && $profileid == 10)	
				$self_assign_flag++;								
		}
		...
		if($self_assign_flag == 0 && $profileid == 10 && $record == "")
			return "outlet_level### Please assign the Target for yourself.";
		
		elseif($self_assign_flag == 2 && $profileid == 10 && $record == "")
			return "outlet_level### Target for this sales person has already been assigned for this period.";	
						
		elseif($total_current_user_target < $total_old_new_target)
			return "outlet_level###UnAllocated No of ROs can not be more than given target of No of ROs.";
			
		elseif($total_current_user_revenue < $total_old_new_revenue)
			return "outlet_level###UnAllocated Revenue can not be more than given target of Revenue.";
