...
class Targets extends CRMEntity {
	...
	var $tab_name_index = Array('vtiger_crmentity'=>'crmid','vtiger_targets'=>'targetsid','vtiger_targetscf'=>'targetsid');
	/**
	...
	var $sortby_fields = Array('targets','smownerid');

	var $list_link_field= 'targets';

	// This is the list of vtiger_fields that are in the lists.
	var $list_fields = Array(
	'First Name' => Array('vtiger_bikescf'=>'cf_697'),
	'Last Name' => Array('vtiger_bikescf'=>'cf_699'),	
	...
	var $required_fields =  array("target"=>1);

	// Used when enabling/disabling the mandatory fields for the module.
	// Refers to vtiger_field.fieldname values.
	var $mandatory_fields = Array('assigned_user_id','target','createdtime' ,'modifiedtime','imagename');
	...
	var $default_order_by = 'target';
	var $default_sort_order = 'ASC';

	// For Alphabetical search
	var $def_basicsearch_col = 'target';

	function Targets() {
		$this->log = LoggerManager::getLogger('target');
...
function get_contactsforol($user_name)
{
  ...
  $log->debug("Exiting get_contactsforol method ...");
	...
	*/
	function save_module($module)
	{
		...
		$salespersonid = $_REQUEST['salesperson'];
		$start_date = $_REQUEST['start_date'];
		$end_date = $_REQUEST['end_date'];
		$target = $_REQUEST['target'];
		$revenue = $_REQUEST['revenue'];		
				
		$adb->pquery("update vtiger_crmentity  set smownerid = ? where crmid=? ",array($salespersonid,$this->id));					
  ...
  */  
	// Start for insert multiple record of Sales Target
		
		if(isset($_REQUEST) && $_REQUEST['record'] == "") {												
			$sub_query1 = " AND vtiger_users.id = ".$salespersonid." ";		
			$userDetails1 = getUserDetails($sub_query1);
			$assigned_email = $userDetails1['email'];
			$fullname1 = $userDetails1['fullname'];
				
			$this->sendMailTarget($fullname1, $start_date, $end_date, $target, $revenue, $assigned_email);
		
			$arr_length = sizeof($_REQUEST);
			$loop_length = floor($arr_length/5);
			if($loop_length > 50)
				$loop_length = 50;
			...
			for($i = 1; $i <= $loop_length; $i++) {		// loop		
				...
				$assignedid = $_REQUEST['salesperson'.$i];
				$revenue = $_REQUEST['revenue'.$i];																				
// Start to send notification 
				$sub_query1 = " AND vtiger_users.id = ".$assignedid." ";		
				$userDetails1 = getUserDetails($sub_query1);
				$assigned_email = $userDetails1['email'];
				$fullname1 = $userDetails1['fullname'];		
// End to send notification				
// Start to stop duplicacy target for the same users
				$permission_target = 0;
				...
				$params = array('Targets', $assignedid, $start_date, $end_date);			
				$result = $adb->pquery($query, $params);
				if ($adb->num_rows($result)) {
					$permission_target = 1;
				}
//End to stop duplicacy target for the same users

				if($target != "" && $assignedid != "" && $start_date != "" && $end_date != "" && $permission_target == 0) {
					$this->sendMailTarget($fullname1, $start_date, $end_date, $target, $revenue, $assigned_email);					
					$crmid = $adb->getUniqueID("vtiger_crmentity");
					$log->debug("Entering into save_module($crmid,$module) method.");
					$createrid = $current_user->id;
					$currentdatetime = date("Y-m-d H:i:s");																											
					$querynum = $adb->query("select prefix, cur_id from vtiger_modentity_num where semodule='Targets' and active = 1");
					$resultnum = $adb->fetch_array($querynum);
					$prefix = $resultnum['prefix'];
					$cur_id = $resultnum['cur_id'];
					$TargetNum = $prefix.$cur_id; 
					$next_curr_id = $cur_id + 1;
					$adb->query("update vtiger_modentity_num set cur_id = ".$next_curr_id." where semodule='Targets' and active = 1");					  					
					$callerName = $TargetNum;
		  			$all_Values = array('target'=>$target,
									'target_no'=> $TargetNum,
									'salesperson'=>$assignedid, 
					...
					$adb->pquery($query, array($crmid, $createrid, $assignedid, $createrid, "Targets", "", $currentdatetime, $currentdatetime, NULL, NULL, 0, 1, 0, $callerName));
					
					$adb->query("INSERT INTO vtiger_targets (targetsid, target, target_no) VALUES(".$crmid.",".$target.", '".$TargetNum."')"); 
										
					$adb->query("INSERT INTO vtiger_targetscf (targetsid, salesperson, start_date, end_date, revenue) VALUES(".$crmid.",".$assignedid.", '".$start_date."', '".$end_date."', '".$revenue."')");
...
// Start Save in Modtracker table ******************
					...
					$adb->pquery('INSERT INTO vtiger_modtracker_basic(id, crmid, module, whodid, changedon, status)
							VALUES(?,?,?,?,?,?)', Array($thisid, $crmid, 'Targets',$current_user->id, date('Y-m-d H:i:s',time()), 2));
...
// End Save in Modtracker table ******************						
				...
				$log->debug("Existing into save_module($crmid,$module) method.");									
				}
			}			
		}				
	// End for insert multiple record of Sales Target
	
	// Start Update All data according to hierarchy ***********					
		$start_date = DateTimeField::__convertToDBFormat($start_date, $date_format);
		// Last day of the month.
		$end_date = date('Y-m-t', strtotime($start_date));				
		$this->updateHierarchyWise($start_date, $end_date);				
// End Update All data according to hierarchy ***********		
		//$this->insertIntoAttachment($this->id,$module);
	}


	function sendMailTarget($fullname1, $start_date, $end_date, $target, $revenue, $assigned_email) {
		global $current_user;
		...
		$fullname2 = $userDetails['fullname'];
		$subject = "New Sales Target";
    
   		 $descriptions = '<table width="80%" cellpadding="5" cellspacing="1" border="0" align="left">
						<tr><td>&nbsp;</td></tr>
						<tr><td>&nbsp;</td></tr>
						<tr><td align="left" width="50%">Dear ('.$fullname1.')</td></tr>
						<tr><td align="left">Following are the Target Details. </td></tr>						
						<tr><td>&nbsp;</td></tr>
						<tr><th align="left">Start Date : </th><td>'.$start_date.'</td></tr>
						<tr><th align="left">End date : </th><td>'.$end_date.'</td></tr>
						<tr><th align="left">Sale Person : </th><td>'.$fullname1.'</td></tr>
						<tr><th align="left">Target : </th><td>'.$target.'</td></tr>
						<tr><th align="left">Revenue : </th><td>'.$revenue.'</td></tr>												
						<tr><td>&nbsp;</td></tr>
						<tr><td>&nbsp;</td></tr>
						<tr><td align="left">Thanks and Regards </td></tr>
						<tr><td align="left">'.$fullname2.'</td></tr>
						</table>';
   		 send_mail('',$assigned_email,$user_name,'',$subject,$descriptions,$cc);
	}


	function updateHierarchyWise($start_date, $end_date) {
		global $adb;
		$query = "select salesperson, vtiger_targets.targetsid as trgtid, outlet, depth, target, cf_781, revenue, 
		vtiger_profile.profileid as currprofileid from vtiger_targets 
		INNER JOIN vtiger_targetscf on vtiger_targetscf.targetsid = vtiger_targets.targetsid
		INNER JOIN vtiger_crmentity on vtiger_crmentity.crmid = vtiger_targets.targetsid 
		INNER JOIN vtiger_users on vtiger_users.id = vtiger_crmentity.smownerid 
		...
		where vtiger_crmentity.deleted = 0 and start_date >= '".$start_date."' AND end_date <= '".$end_date."' ";		
			...
			$target_qry = $adb->query($query);		
			if($adb->num_rows($target_qry) > 0) {
				$total_target_6 = 0;
				$total_sub_target_4 = 0;
				$total_revenue_6 = 0;
				$total_sub_revenue_4 = 0;
				while($row = $adb->fetch_array($target_qry)) {
					$new_depth = $row['depth'];
					$profileid = $row['currprofileid'];
					$targetsid = $row['trgtid'];
					$zonenew = $row['cf_781'];
					$target = $row['target'];
					$revenue = $row['revenue'];
					...
					if($profileid == 10) { // 10 Head BD Profile
						$parent_targetid = $targetsid;
						$parent_total_target = $target;
						$parent_total_revenue = $revenue;
					}
					elseif($new_depth == 2) { //2 RBDM  Profile
						if($zonenew == 'East') {
							$parent1_targetid_east = $targetsid;
							$parent1_total_target_east = $target;
							$parent1_total_revenue_east = $revenue;
						}
						elseif($zonenew == 'West') {
							$parent1_targetid_west = $targetsid;
							$parent1_total_target_west = $target;
							$parent1_total_revenue_west = $revenue;
						}
						elseif($zonenew == 'North') {
							$parent1_targetid_north = $targetsid;
							$parent1_total_target_north = $target;
							$parent1_total_revenue_north = $revenue;
						}
						else {
							$parent1_targetid_south = $targetsid;
							$parent1_total_target_south = $target;
							$parent1_total_revenue_south = $revenue;
						}
						$total_sub_target_4 = $target + $total_sub_target_4;
						$total_sub_revenue_4 = $revenue + $total_sub_revenue_4;						
					}
					else{
						if($zonenew == 'East') {
							$total_sub_target_6_east = $target + $total_sub_target_6_east;
							$total_sub_revenue_6_east = $revenue + $total_sub_revenue_6_east;	
						}
						elseif($zonenew == 'West') {
							$total_sub_target_6_west = $target + $total_sub_target_6_west;
							$total_sub_revenue_6_west = $revenue + $total_sub_revenue_6_west;	
						}
						elseif($zonenew == 'North') {
							$total_sub_target_6_north = $target + $total_sub_target_6_north;
							$total_sub_revenue_6_north = $revenue + $total_sub_revenue_6_north;	
						}
						else {
							$total_sub_target_6_south = $target + $total_sub_target_6_south;
							$total_sub_revenue_6_south = $revenue + $total_sub_revenue_6_south;	
						}
					}
				}
						
				$parent_unallocated_target =  $parent_total_target - $total_sub_target_4;
				$parent_sub_unallocated_target_east = $parent1_total_target_east - $total_sub_target_6_east;
				$parent_sub_unallocated_target_west = $parent1_total_target_west - $total_sub_target_6_west;
				$parent_sub_unallocated_target_north = $parent1_total_target_north - $total_sub_target_6_north;
				$parent_sub_unallocated_target_south = $parent1_total_target_south - $total_sub_target_6_south;
				
				$parent_unallocated_revenue =  $parent_total_revenue - $total_sub_revenue_4;
				$parent_sub_unallocated_revenue_east = $parent1_total_revenue_east - $total_sub_revenue_6_east;
				$parent_sub_unallocated_revenue_west = $parent1_total_revenue_west - $total_sub_revenue_6_west;
				$parent_sub_unallocated_revenue_north = $parent1_total_revenue_north - $total_sub_revenue_6_north;
				$parent_sub_unallocated_revenue_south = $parent1_total_revenue_south - $total_sub_revenue_6_south;
				
				$adb->query("UPDATE vtiger_targetscf SET  
				unallocated_ro = ".$parent_unallocated_target.", unallocated_revenue = ".$parent_unallocated_revenue."  
				where targetsid = ".$parent_targetid." ");
				$adb->query("UPDATE vtiger_targetscf SET  
				unallocated_ro = ".$parent_sub_unallocated_target_east." , unallocated_revenue = ".$parent_sub_unallocated_revenue_east."   
				where targetsid = ".$parent1_targetid_east." ");
				$adb->query("UPDATE vtiger_targetscf SET  
				unallocated_ro = ".$parent_sub_unallocated_target_west." , unallocated_revenue = ".$parent_sub_unallocated_revenue_west."    
				where targetsid = ".$parent1_targetid_west." ");
				$adb->query("UPDATE vtiger_targetscf SET  
				unallocated_ro = ".$parent_sub_unallocated_target_north." , unallocated_revenue = ".$parent_sub_unallocated_revenue_north."   
				where targetsid = ".$parent1_targetid_north." ");
				$adb->query("UPDATE vtiger_targetscf SET  
				unallocated_ro = ".$parent_sub_unallocated_target_south." , unallocated_revenue = ".$parent_sub_unallocated_revenue_south."   
				where targetsid = ".$parent1_targetid_south." ");
			}
	}
