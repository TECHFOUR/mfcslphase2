...
class Campaigns extends CRMEntity {
	...
	var $search_fields = Array(
			'Campaign No'=>Array('vtiger_campaign'=>'campaignname'),
	...
	*/
	function save_module($module)   // New Code added by jitendra singh [TECHFOUR]
	{
		...
		$adb->query("update vtiger_campaign set campain_day = '".$daymain."', expectedresponse = 'Planned' where campaignid = ".$this->id."");
		...
		if(!empty($_REQUEST['currentid'])){    // Update for the Day of main screen						
				...
				if($_REQUEST['end_date'] == ''){
					list($year, $month, $day2) = split('[/.-]', $startingdate);
		...
		for($i = 0; $i <= $subArray_length; $i++) {
			...
			if($i == 0)
				...
				if($end_date == ''){
				list($year, $month, $day1) = split('[/.-]', $start_date);
				$start_date1 = $year."-".$month."-"."1";
				$end_date = date('Y-m-d',strtotime($start_date1 . "+1 month -1 second"));
				}
	...
	$adb->pquery($query, array($crmid, $createrid, $assignedid, $createrid, "Campaigns", "", $currentdatetime, $currentdatetime, NULL, NULL, 0, 1, 0, $callerName));
		  ...
		  $adb->query("INSERT into vtiger_campaignscf (campaignid,end_date,start_time,end_time,location,leaflet,poster,tag,other1,other2) values(".$crmid.",'".$end_date."','".$start_time."','".$end_time."','".$location."','".$leaflet."','".$poster."','".$tag."','".$other1."', '".$other2."')"); 		  
