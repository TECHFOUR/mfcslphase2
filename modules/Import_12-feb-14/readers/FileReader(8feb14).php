...
class Import_FileReader_Reader {
	...
	if($_REQUEST['module'] == 'Leads'){
		...
		$current_outlet_qry = $db->query("select vtiger_outletmastercf.outletmasterid from vtiger_outletmastercf inner join vtiger_outletmaster on  vtiger_outletmaster.outletmasterid = vtiger_outletmastercf.outletmasterid where outlet = '".rtrim(ltrim($fieldValues[26]))."' ");
		$outletdata = $db->fetch_array($current_outlet_qry);
       	$currentoutlet = $outletdata['outletmasterid'];
		$outlet_id = $currentoutlet;
		$_SESSION['central_outlet_id'] = $outlet_id;
		}else{
		...
		if($fieldValues[0] != '' && $db->num_rows($duplicateqry) == 0 && strlen($fieldValues[0]) == 10 && ($fieldValues[1] != "" &&  $fieldValues[2] != "") && $firsttime_mobile != 1 && $check_lead_exist == 1 && $temp_mobile_exist == 0){
			...
			$db->pquery('INSERT INTO '.$tableName.' ('. implode(',', $columnNames).') VALUES ('. generateQuestionMarks($fieldValues) .')', $fieldValues);
				...
				if($db->num_rows($duplicateqry) > 0 || $temp_mobile_exist == 1)
				...
				if($fieldValues[0] != '' && strlen($fieldValues[0]) == 10 && $fieldValues[1] != '' && $fieldValues[2] != ''){
					$logerror = "";
					$myarray = array(
					...
					if($mobileexist == 1 && $fieldValues[24] == '' && $new_campaign_priority <= $old_campaign_priority){
					...
					if($fieldValues[24] == '' && $check_lead_exist == 0){
					...
					elseif($mobileexist == 1 && $fieldValues[24] != '' && $new_campaign_priority > $old_campaign_priority){
					...
					elseif($firsttime_mobile == 1 && $fieldValues[24] != '' && $firsttime_mobile == 1){
