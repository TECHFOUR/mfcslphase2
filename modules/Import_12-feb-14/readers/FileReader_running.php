...
// Start New Conditon check by Ajay ***************************	
		...
		if($lead_create_update_status == 1 && $fieldValues[0] != ''){  //create			
			$this->createNewLead($fieldValues,$campaignlocation,$camp_type_id,$campcategory,$make_id,$model_id);
				
		}
			
		elseif($lead_create_update_status == 2 && $fieldValues[0] != '') { // Update		
	...
	function createNewLead($fieldValues,$campaignlocation,$camp_type_id,$campcategory,$make_id,$model_id) {
		...
		$adb->query("insert into vtiger_leaddetails (leadid,firstname,lastname,email,company,designation,secondaryemail,outlet)
 ...
 $adb->query("insert into vtiger_leadsubdetails(leadsubscriptionid,campaignlocation)values(".$crmid.",'".$campaignlocation."')"); 
		...
		$adb->query("insert into vtiger_leadscf (leadid,society_name,homeaddtwo,homeaddthree,home_state,officeaddtwo,officeaddthree,office_state,dateofbirth,   occupation,registrationno,insurancedate,insurancecompany,odometer,lastservicedate,dateofsale,campaign_category,make,model)
...
,'".$fieldValues[31]."','".$campcategory."',".$make_id.",".$model_id.")");
