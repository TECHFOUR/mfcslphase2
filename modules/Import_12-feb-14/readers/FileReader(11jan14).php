...
class Import_FileReader_Reader {
	...
	if($_REQUEST['module'] == 'Leads'){
		...
		$campaignqry = mysql_query("select campaignname from vtiger_campaign inner join vtiger_crmentity on  vtiger_crmentity.crmid = vtiger_campaign.campaignid 
		where vtiger_crmentity.deleted = 0");
		
		 while ($rowdata = mysql_fetch_array($campaignqry)) {
		...
		if($fieldValues[0] != '' && mysql_num_rows($duplicateqry) == 0 && strlen($fieldValues[0]) == 10 && ($fieldValues[1] != "" &&  $fieldValues[2] != "")){
			...
			$db->pquery('INSERT INTO '.$tableName.' ('. implode(',', $columnNames).') VALUES ('. generateQuestionMarks($fieldValues) .')', $fieldValues);
				...
				fputcsv($handle, array($logerror,$fieldValues[0],$fieldValues[1],$fieldValues[2],$fieldValues[3],$fieldValues[4],$fieldValues[5],$fieldValues[6],$fieldValues[7],$fieldValues[8],$fieldValues[9],
				...
				$fieldValues[21],$fieldValues[22],$fieldValues[23],$fieldValues[24],$fieldValues[25],$fieldValues[26],$fieldValues[27],$fieldValues[28],$fieldValues[29],$fieldValues[30],$fieldValues[31]));
			}
	}
	else{
		$db->pquery('INSERT INTO '.$tableName.' ('. implode(',', $columnNames).') VALUES ('. generateQuestionMarks($fieldValues) .')', $fieldValues);
