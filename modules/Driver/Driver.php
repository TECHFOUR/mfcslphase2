...
class Driver extends CRMEntity {
	...
	function save_module($module) {
		...
		$drivermasterid = $_REQUEST['cf_877'];
		$driver_qry = $adb->query("select cf_891 from vtiger_drivermastercf where drivermasterid = ".$drivermasterid."");	
		
  		if($adb->num_rows($driver_qry) > 0) {
			$result = $adb->fetch_array($driver_qry);
			$mobile = $result['cf_891'];
			$adb->query("update vtiger_drivercf set cf_879 = ".$mobile."  where driverid = ".$this->id."");			
		}
