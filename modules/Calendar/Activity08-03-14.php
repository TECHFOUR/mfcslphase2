...
// Start to Transfer Lead to other Outlet by Agents	
	if($assigned_to_outlet == 'on' && $other_outletid != "") {
		...
		$this->createNewActivity($subject, $date_start, $time_start, $time_end, $app_book_date, $app_book_time, $driver_pickup, $sub_disposition, $old_leadstatus, $activity_status, $cro_id);
	}
// End to Transfer Lead to other Outlet by Agents					
	...
	function createAppointmentLeadActivity($status, $old_leadstatus, $day_counter_flag, $start_date_counter) {
		...
		if($driver_pickup == 0 && $disposition == 0) { // if No Driver									 			 
			 $this->createNewActivity($callerName, $date_start, $time_start, $time_end, $app_book_date, $app_book_time, $driver_pickup, $sub_disposition, $old_leadstatus, $status, $assigned_user_id);
			 	$leadDetails = $this->getLeadDetails($parent_id);
				$campaign_type = $leadDetails['campaign_type'];				
				if($profileid == 3) { // Central User
	...
	function createNewActivity($callerName, $date_start, $time_start, $time_end, $app_book_date, $app_book_time, $driver_pickup, $sub_disposition, $old_leadstatus, $status, $new_assigned_to) {
		...
		$adb->query("INSERT into vtiger_activitycf (activityid, app_book_date, app_book_time, driver_pickup, cf_903, cf_895, cf_951, assigned_to_outlet) 
					values(".$crmid.",'".$app_book_date."','".$app_book_time."','".$driver_pickup."','".$sub_disposition."','".$status."','".$other_outletid."','".$outlet_other."')"); 
