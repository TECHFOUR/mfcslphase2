...
class Activity extends CRMEntity {
	...
	{		
		...
		if($profileid == 3 || $profileid == 6 || $profileid == 5){			
			$adb->query("update vtiger_crmentity  set smownerid = ".$current_user->id." where crmid= ".$this->id."");
			$assignedid = $current_user->id;
			$this->column_fields['assigned_user_id'] = $assignedid;
		}
		//echo $assignedid.'__'.$this->column_fields['assigned_user_id'].'___'.$this->column_fields['parent_id'];die;
		//echo "<pre>";print_r($lead_details);die;
		//Handling module specific save
		//Insert into seactivity rel
		//echo "<pre>";print_r($this->column_fields);die;
				
		$adb->pquery("update vtiger_crmentity  set smownerid = ? where crmid=? ",array($assignedid,$this->id));
...
// Start Auto Lead activity [TECHFOUR]  **********************
		...
		$driver_pickup_flag = $this->column_fields['driver_pickup'];
		
		$assigned_to_outlet = $this->column_fields['assigned_to_outlet'];
		$other_outletid = $this->column_fields['cf_951'];
			
		$leadDetails = $this->getLeadDetails($parent_id);
		$setype = $leadDetails['setype'];
		$old_leadstatus = $leadDetails['leadstatus'];
		$day_counter_flag = $leadDetails['day_counter_flag'];
		$start_date_counter = $leadDetails['start_date_counter'];
		$customer_type = $leadDetails['leadsource'];
...
// Start to Transfer Lead to other Outlet by Agents	
	/*if($assigned_to_outlet == 'on' && $other_outletid != "") {
		$driver_pickup = 0;		
		if($driver_pickup_flag == "on")
			$driver_pickup = 1;
			
		$sub_query_CRO = " AND vtiger_outletmastercf.outletmasterid = ".$other_outletid."  AND vtiger_role2profile.profileid = 5 ";   		
		$userDetails_CRO = getUserDetails($sub_query_CRO);
		$cro_id = $userDetails_CRO['id'];
		$this->setReassignedLead($parent_id, $cro_id, $assigned_user_id);
		$this->createNewActivity($subject, $date_start, $time_start, $time_end, $app_book_date, $app_book_time, $driver_pickup, $sub_disposition, $old_leadstatus, $activity_status, $cro_id, $activity_status);
	}*/
// End to Transfer Lead to other Outlet by Agents					
	}	
	...
	function createAppointmentLeadActivity($status, $old_leadstatus, $day_counter_flag, $start_date_counter) {
		...
		$sub_query_CRO = " AND vtiger_outletmastercf.outletmasterid = ".$outletmasterid."  AND vtiger_role2profile.profileid = 5 ";   		
		$userDetails_CRO = getUserDetails($sub_query_CRO);
		$cro_id = $userDetails_CRO['id'];				
		$driver_pickup = 0;
		...
		switch($sub_disposition) {
			...
			case "Rescheduled":
				...
				if(isset($_REQUEST['record']) && $_REQUEST['record'] != "")
					$adb->query("UPDATE vtiger_activity SET eventstatus = 'Held' WHERE vtiger_activity.activityid = ".$_REQUEST['record']." ");
			break;
			...
			if($driver_pickup == 0) {
				...
				$new_date_start2 = strtotime('+1 days', strtotime($new_date_start1));
				$new_app_book_date = strtotime($new_app_book_date1);				
				if($new_date_start2 == $new_app_book_date) {
					$time_start = "9:00:00";	
					$time_end = "9:05:00";
					$date_start = date('Y-m-d',$new_app_book_date);
					$app_book_date = $date_start;
				}else {
					$new_app_book_date = date('Y-m-d', strtotime('-1 days', strtotime($new_app_book_date1)));
					$date_start = $new_app_book_date;
					$app_book_date = date('Y-m-d',strtotime($new_app_book_date1));
					$time_start = "17:00:00";	
					$time_end = "17:05:00";	
				}
			}else { // driver pickup
		...
		//echo $driver_pickup.'___'.$disposition.'___'.$profileid;die;
		
		if($driver_pickup == 0 && $disposition == 0) { // if No Driver									 			 
			 $this->createNewActivity($callerName, $date_start, $time_start, $time_end, $app_book_date, $app_book_time, $driver_pickup, $sub_disposition, $old_leadstatus, $status, $assigned_user_id, $status);
			 	$leadDetails = $this->getLeadDetails($parent_id);
				$campaign_type = $leadDetails['campaign_type'];				
				if($profileid != 3 && ($assigned_to_outlet != 'on' && $other_outletid == "")) { // For no Central User
					$adb->query("UPDATE vtiger_leaddetails 
						...
						SET  smownerid = ".$assigned_user_id.", modifiedby = ".$current_user->id.", modifiedtime = 
						'".date("Y-m-d H:i:s",time())."' where vtiger_leaddetails.leadid = ".$parent_id." ");	
				}
		}
		
		if($driver_pickup == 1 && $disposition == 0 ) { // if Driver						
			 // Re-Assignment
			 $new_assigned_to = $cro_id;
			 if($assigned_user_id != $cro_id && $profileid == 6) {
				 $this->setReassignedLead($parent_id, $cro_id, $assigned_user_id);				
			 }			 
			 $this->createNewActivity($callerName, $date_start, $time_start, $time_end, $app_book_date, $app_book_time, $driver_pickup, $sub_disposition, $old_leadstatus, $status, $new_assigned_to, $status);			 
		}
	...
	function createRescheduledLeadActivity($status, $old_leadstatus, $day_counter_flag, $start_date_counter) {
		...
		$start_time_1 = strtotime("09:00:00");
		$end_time_1 = strtotime("12:00:00");
		
		$start_time_2 = strtotime("12:00:01");
		$end_time_2 = strtotime("15:00:00");
		
		if($start_time_1 <= $start_time && $end_time <= $end_time_1) {
			$time_start = "12:05:00";
			$time_end = "12:10:00";	
		}
		else if($start_time_2 <= $start_time && $end_time <= $end_time_2) {
			$time_start = "15:05:00";
			$time_end = "15:10:00";	
		}
		else {
		...
		$new_assigned_to = $assigned_user_id;
		$this->createNewActivity($callerName, $new_date_start, $time_start, $time_end, $app_book_date, $app_book_time, $driver_pickup, $sub_disposition, $old_leadstatus, $status, $new_assigned_to, 'Mobile not Reachable OR Switched Off');
	...
	function createNewActivity($callerName, $date_start, $time_start, $time_end, $app_book_date, $app_book_time, $driver_pickup, $sub_disposition, $old_leadstatus, $status, $new_assigned_to, $activitystatus) {
		global $adb, $current_user;
		...
		$userDetails = getUserDetails($sub_query);				
		$profileid = $userDetails['profileid'];	
		//echo $old_leadstatus.'++++'.$status;die;	
		if($profileid == 3 && $status == "Appointment Booked") { // For Central if Campaign Type InBound then transfer to OutLet CRO
			$leaddetails = $this->getLeadDetails($parent_id);
			$outletmasterid = $leaddetails['outlet'];					
			$sub_query_CRO = " AND vtiger_outletmastercf.outletmasterid = ".$outletmasterid."  AND vtiger_role2profile.profileid = 5 ";   		
			$userDetails_CRO = getUserDetails($sub_query_CRO);
			$cro_id = $userDetails_CRO['id'];				
			$this->setReassignedLead($parent_id, $cro_id, $assigned_user_id);
			$assigned_user_id = $cro_id;			
		}

// Start to Transfer Lead to other Outlet by Agents
        if($assigned_to_outlet == 'on' && $other_outletid != "" && $status == "Appointment Booked") {
            $sub_query_CRO = " AND vtiger_outletmastercf.outletmasterid = ".$other_outletid."  AND vtiger_role2profile.profileid = 5 ";
            $userDetails_CRO = getUserDetails($sub_query_CRO);
            $cro_id = $userDetails_CRO['id'];
            $this->setReassignedLead($parent_id, $cro_id, $assigned_user_id);
...
// End to Transfer Lead to other Outlet by Agents
		...
		$adb->query("INSERT into vtiger_activitycf (activityid, app_book_date, app_book_time, driver_pickup, cf_903, cf_895, assigned_to_outlet)
					values(".$crmid.",'".$app_book_date."','".$app_book_time."','".$driver_pickup."','".$sub_disposition."','".$activitystatus."', '".$outlet_other."')");
		...
		//if($old_leadstatus != $status) {
			...
			$adb->query("UPDATE vtiger_leaddetails INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_leaddetails.leadid 
						...
						SET app_book_date = '".$app_book_date."', app_book_time = '".$app_book_time."',
						leadstatus = '".$status."', modifiedby = ".$current_user->id.", modifiedtime = '".date('Y-m-d H:i:s')."' ,
						latest_activity_date = '".$date_start."'
						WHERE vtiger_leaddetails.leadid = ".$parent_id." ");
		/*}else {
	...
	function getLeadDetails($parent_id) {		
		...
		$query = "SELECT vendorname, outlet, setype, leadstatus, day_counter_flag, start_date_counter, leadsource  FROM vtiger_crmentity 
		...
		if($num_rows > 0) {	
			...
			$lead_details['leadstatus'] = $adb->query_result($result,$i,'leadstatus');
			$lead_details['day_counter_flag'] = $adb->query_result($result,$i,'day_counter_flag');
			$lead_details['start_date_counter'] = $adb->query_result($result,$i,'start_date_counter');
			$lead_details['leadsource'] = $adb->query_result($result,$i,'leadsource');
			$lead_details['outlet'] = $adb->query_result($result,$i,'outlet');
			$lead_details['campaign_type'] = $adb->query_result($result,$i,'vendorname');					
		}
		$log->debug("Exiting getLeadDetails method ...");		
		return $lead_details;
	}
	
	function setReassignedLead($parent_id, $cro_id, $assigned_user_id) {
		global $adb, $current_user;
        $sub_query = " AND vtiger_users.id = ".$cro_id." ";
        $userDetails = getUserDetails($sub_query);
        $outletmasterid = $userDetails['outletmasterid'];
		...
		$adb->query("UPDATE vtiger_leaddetails 
			...
			INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_leaddetails.leadid
			INNER JOIN vtiger_leadscf ON vtiger_leadscf.leadid = vtiger_leaddetails.leadid
			SET  outlet = ".$outletmasterid.", plant_code = '".$plantcode."',
			smownerid = ".$cro_id.", modifiedby = ".$current_user->id.", modifiedtime =
			'".date("Y-m-d H:i:s",time())."' where vtiger_leaddetails.leadid = ".$parent_id." ");                                       
	}
