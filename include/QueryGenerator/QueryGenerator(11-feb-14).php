...
// Start added by Ajay [TECHFOUR] for Condition listing in Sales Target Module
	if($_REQUEST['src_module'] == "Targets") {
		global $adb;		
		$userid = $_SESSION['authenticated_user_id'];
		require('user_privileges/user_privileges_'.$userid.'.php');	
		$current_user_roles;
		$is_admin = $user_info['is_admin'];	
		$roledepth_qry = $adb->query("select depth, rolename from vtiger_role where roleid = '".$current_user_roles."' ");		
  		if($adb->num_rows($roledepth_qry) > 0) {
			$row = $adb->fetch_array($roledepth_qry);
			$role_depth = $row['depth'];
			$zone_qry = $adb->query("select cf_781 from vtiger_users INNER JOIN vtiger_outletmastercf on vtiger_outletmastercf.outletmasterid = vtiger_users.cf_775 where id = ".$userid." ");
			if($adb->num_rows($zone_qry) > 0) {
				$row = $adb->fetch_array($zone_qry);
				$zone = $row['cf_781'];
			}
		}
	}
...
// Start added by Ajay [TECHFOUR] for Condition listing in Sales Target Module			
			if($_REQUEST['src_module'] == "Targets" && $role_depth > 1 && $is_admin == "off")
				$query .= "  INNER JOIN vtiger_user2role ON vtiger_users.id = vtiger_user2role.userid INNER JOIN vtiger_role on vtiger_role.roleid = vtiger_user2role.roleid INNER JOIN vtiger_outletmastercf on vtiger_outletmastercf.outletmasterid = vtiger_users.cf_775 ";
// End			
...
// Start added by Ajay [TECHFOUR] for Condition listing in Sales Target Module						
			if($_REQUEST['src_module'] == "Targets" && $role_depth > 1 && $is_admin == "off") {
				if($role_depth == 2)				
					$custom_qry = " and (depth in(5,2) AND vtiger_outletmastercf.outlet like '%Corporate%') OR vtiger_outletmastercf.outlet like '%RBDM%'  ";
				if($role_depth == 5)
					$custom_qry = " and depth in(8) AND rolename like '%MO%' AND vtiger_outletmastercf.cf_781 like '%".$zone."%' ";
					
				$query .= $custom_qry;
			}
// End

// Start to add active Man Power condition [TECHFOUR]
			if($_REQUEST['module'] == "DriverMaster" && $_REQUEST['view'] == "Popup" && ($_REQUEST['src_module'] == "Driver" || $_REQUEST['src_module'] == "Leads")) {
				$query .= " AND vtiger_drivermastercf.cf_893 = '1'";
			}
// End to add active Man Power condition [TECHFOUR]  

// Start to add active Man Power condition [TECHFOUR]
			if($_REQUEST['module'] == "Services" && $_REQUEST['src_module'] == "Leads" && $_REQUEST['view'] == "Popup") {
				$id = $_COOKIE['the_cookie'];
				$query .= " AND vtiger_service.make = ".$id." ";
			}
// End to add active Man Power condition [TECHFOUR]  

// Start to add active Man Power condition [TECHFOUR]
			if($_REQUEST['module'] == "ServiceContracts" && $_REQUEST['src_module'] == "Campaigns") {
				$query .= " AND vtiger_servicecontracts.contract_status = 'Active' ";
			}
// End to add active Man Power condition [TECHFOUR]

// Start to add Approved campaign condition Jitendra Singh on 24 Jan 2014[TECHFOUR] 
			if($_REQUEST['module'] == "Campaigns" && $_REQUEST['src_module'] == "Leads" && $_REQUEST['view'] == "Popup") {
				$query .= " AND vtiger_campaign.campaignstatus = 'Approved' AND vtiger_campaign.expectedresponse = 'Held' ";
			}
// End to add Approved campaigncondition [TECHFOUR]
			$this->query = $query;
			return $query;
		} else {
			return $this->query;
