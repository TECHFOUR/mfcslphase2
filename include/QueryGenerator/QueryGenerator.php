...
// Start added by Ajay [TECHFOUR] for Condition listing in Sales Target Module
	if($_REQUEST['src_module'] == "Targets" || $_REQUEST['src_module'] == "CallingMaster" || $_REQUEST['src_module'] == "Calendar") {
		global $adb;		
...
// Start added by Ajay [TECHFOUR] for Condition listing in Sales Target Module			
			if(($_REQUEST['src_module'] == "Targets" || $_REQUEST['src_module'] == "CallingMaster") && $profileid > 1 && $is_admin == "off")
				$query .= "  INNER JOIN vtiger_user2role ON vtiger_users.id = vtiger_user2role.userid 
...
// Start added by Ajay [TECHFOUR] for Condition listing in Sales Target Module						
			if($_REQUEST['src_module'] == "Targets" && $profileid > 1 && $is_admin == "off") {
			...
			if($_REQUEST['src_module'] == "CallingMaster" && $profileid > 1 && $is_admin == "off") {
				$query .= " AND vtiger_profile.profileid IN(3, 5, 6) AND vtiger_outletmastercf.outletmasterid = $outletmasterid ";
			}
// End
//echo $query;
...
// End to add Approved campaigncondition [TECHFOUR]
			...
			// Start to add Approved campaign condition Jitendra Singh on 24 Jan 2014[TECHFOUR] 
			if($_REQUEST['module'] == "OutletMaster" && $_REQUEST['src_module'] == "Calendar" && $_REQUEST['view'] == "Popup") {
				$query .= " AND outlet NOT IN('RBDM', 'Business Manager', 'BD Corporate', 'Tele Caller', 'Corporate', 'Regional Manager') 
							AND vtiger_outletmaster.outletmasterid <> $outletmasterid ";
			}
// End to add Approved campaigncondition [TECHFOUR]
