...
function getDuplicateLeadDetails($registrationno, $mobile, $ststus = "") {
		...
		$query = "SELECT outletmaster, vtiger_leadaddress.campaignid as campid, campaignlocation,lead_no, vtiger_vendorcf.priority 
				...
				old_camp_type_id, vtiger_leadscf.campaign_category as campcategory, 
				vtiger_outletmaster.outletmasterid as outletid FROM vtiger_leadscf 
				...
				if($ststus == 'without_regno') {
					$query .= "AND mobile = ? ";
					$params = array('Leads', $mobile);
				}else {
					$query .= "AND (registrationno = ?  AND mobile = ?)";
					$params = array('Leads', $registrationno, $mobile);	
				}				
		...
		for($i=0; $i<$num_rows; $i++) {	
			...
			$lead_details[$i]['outletid'] = $adb->query_result($result,$i,'outletid');
