...
function getUserDetails($sub_query) {		
		global $adb, $log;
		$log->debug("Entering getUserDetails() method ...");		
		$query = "SELECT outletmaster, profileid, vtiger_outletmastercf.outletmasterid as outletid, cf_781, depth, id, concat(first_name,' ',last_name) as fullname, user_name, email1 , vtiger_role.roleid as roleid, vtiger_role.rolename as rolename FROM vtiger_users 
	...
	WHERE status = 'Active' ";
		...
		$user_details=Array();
		$profile_permission = 4;
		$i=0;		
		if($num_rows > 0) {
			$profileid = $adb->query_result($result,$i,'profileid');
			if($profileid == 7 || $profileid == 10 || $profileid == 12) // // Region Level
				$profile_permission = 1;
			elseif($profileid == 2 || $profileid == 8 || $profileid == 9) // Manager level
				$profile_permission = 2;			
			elseif($profileid == 4 || $profileid == 11 || $profileid == 13) // Mo level
				$profile_permission = 3;			
			elseif($profileid == 3 || $profileid == 5 || $profileid == 6 || $profileid == 1) // agent level
				$profile_permission = 4;
				
			$user_details['id'] = $adb->query_result($result,$i,'id');		
			$user_details['user_name'] = $adb->query_result($result,$i,'user_name');
			$user_details['fullname'] = $adb->query_result($result,$i,'fullname');
			$user_details['email'] = $adb->query_result($result,$i,'email1');
			$user_details['roleid'] = $adb->query_result($result,$i,'roleid');
			$user_details['rolename'] = $adb->query_result($result,$i,'rolename');
			$user_details['zone'] = $adb->query_result($result,$i,'cf_781');
			$user_details['depth'] = $adb->query_result($result,$i,'depth');
			$user_details['profileid'] = $profileid;
			$user_details['outletmasterid'] = $adb->query_result($result,$i,'outletid');
			$user_details['outletmaster'] = $adb->query_result($result,$i,'outletmaster');
			$user_details['profile_permission'] = $profile_permission;		
		}
		$log->debug("Exiting getUserDetails method ...");		
		return $user_details;
	}
...
function getSharingModuleList($eliminateModules=false)
{
	...
	// Module that needs to be eliminated explicitly [TECHFOUR]
